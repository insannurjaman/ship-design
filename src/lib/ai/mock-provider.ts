...
function createMockArtifactText(prompt: string) {
  ...
  if (/Create the Handoff Docs artifact/i.test(prompt)) {
    return [
      ...
      "- Targeted regeneration flow",
      "- Provider fallback recovery flow",
      "",
      "## Design System",
      "- Use dark technical surfaces, acid green accents, 1px borders, and mono labels.",
      "",
      "## UI Screens",
      "- Recreate screen specs manually in Figma using the UI Screens artifact.",
      "",
      "## QA Checklist",
  ...
  return [
    ...
    "- Output review, copy, export, and local Figma package prep",
    "",
    "## Success Signals",

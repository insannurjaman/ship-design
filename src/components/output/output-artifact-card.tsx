...
export function OutputArtifactCard({ output, state = "ready" }: OutputArtifactCardProps) {
  const isSkipped = output.status === "skipped";
  const isReady = state !== "waiting" && !isSkipped;
  ...
  return (
    <article className="border border-line bg-surface-base p-4 transition-colors hover:border-line-strong hover:bg-surface-hover">
      <Badge tone={isReady ? "success" : "muted"}>{isSkipped ? "skipped" : isReady ? state : "waiting"}</Badge>
      ...
      <p className="mt-2 text-sm leading-6 text-ink-secondary">
        {isSkipped ? output.summary : isReady ? output.summary : "This artifact appears after the relevant agent completes."}
      </p>
    </article>
  );
}
